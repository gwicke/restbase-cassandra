...
class DB {
    ...
    _deriveHashTableSchemas(schema) {
        const topIdx = schema.index.filter((idx) => idx.type !== 'range');
        let workIdx = topIdx;
        const rangeIndexes = schema.index.filter((idx) => idx.type === 'range');
        const idxVariants = [topIdx];
        rangeIndexes.forEach((rangeKey) => {
            const hashKey = Object.assign({}, rangeKey);
            hashKey.type = 'hash';
            delete hashKey.order;
            workIdx = workIdx.slice();
            workIdx.push(hashKey);
            idxVariants.push(workIdx);
        });

        // Lowest / all-key level should be hash_0.
        idxVariants.reverse();
        const res = [];
        for (let i = 0; i < idxVariants.length; i++) {
            res.push(dbu.makeSchemaInfo(Object.assign({}, schema,
                {
                    _columnfamily: `hash_${i}`,
                    index: idxVariants[i],
                    secondaryIndexes: {}
                }
            )));
    ...
    createTable(domain, query) {
        ...
        .then((req) => {
            ...
            const hashTableSchemas = this._deriveHashTableSchemas(newSchema);
            // console.log(JSON.stringify(hashTableSchemas, null, 2));
