...
class InternalRequest {
    constructor(opts) {
        ...
        this.columnfamily = opts.columnfamily || 'hash';
...
class DB {
    ...
    createTable(domain, query) {
        ...
        .then((req) => {
            ...
            const doCreateTables = () => {
                ...
                .then(() => this._createTable(req, newSchemaInfo, 'hash'))
    ...
    _getSchema(keyspace) {
        return this.client.metadata.getTable(keyspace, 'hash')
            .then((info) => {
                if (!info) {
    ...
    _putSchema(req, newSchema) {
        ...
        const table = `${dbu.cassID(req.keyspace)}.hash`;
