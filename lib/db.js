...
class DB {
    ...
    _hashTableLevel(schemaInfo, query) {
        if (schemaInfo && schemaInfo.cqlTables) {
            // Check how many index attributes are missing.
            const iKeys = schemaInfo.cqlTables.hash_0.iKeys;
    ...
    get(domain, query) {
        ...
        .then((req) => {
            ...
            // FIXME: Handle table selection here (or in makeInternalRequest)
            // if (query.index) {
            ...
            .then((res) => {
                ...
                res.items = dbu.convertRows(res.items,
                    req.schema.cqlTables[req.columnfamily]);
                return res;
            });
        });
    }
    ...
    _getRaw(req, options) {
        ...
        const schema = req.schema && req.schema.cqlTables[req.columnfamily];
        if (!schema) {
            throw new Error(`restbase-mod-table-cassandra: `
        ...
        if (!schema.iKeyMap) {
            this.log('error/cassandra/no_iKeyMap', schema);
        }
        ...
        const buildResult = dbu.buildGetQuery(req, req.schema.cqlTables[req.columnfamily],
            options);
    ...
    _put(req) {
        if (!req.schema) {
            throw new Error(`put: Schema for table ${req.table} not found!`);
        }
        ...
        if (schema.secondaryIndexes) {
            Object.keys(schema.secondaryIndexes).forEach((idx) => {
                ...
                batch.push(dbu.buildPutQuery(idxReq, secondarySchema));
            });
        }


        // Write to upper-level hash tables first
        const cqlTableNames = Object.keys(schema.cqlTables);
        if (cqlTableNames.length) {
            for (let i = 1; i < cqlTableNames.length; i++) {
                batch.push(dbu.buildPutQuery(req, schema.cqlTables[`hash_${i}`]));
            }
        }
        ...
        batch.push(dbu.buildPutQuery(req, req.schema.cqlTables['hash_0']));
    ...
    _deleteOlderContent(req) {
        ...
        const dataGetInfo = dbu.buildGetQuery(dataGetReq, dataGetReq.schema, { limit: 3 });
        return dbu.eachRow(
            this.client,
            dataGetInfo.cql,
            dataGetInfo.params,
    ...
    _backgroundUpdates(req, limit, indexes) {
        ...
        .then(() => {       // Query for 'limit' subsequent records
            ...
            const dataGetInfo = dbu.buildGetQuery(dataGetReq,
                dataGetReq.schema, { withTTL: true, limit });
    ...
    createTable(domain, query) {
        ...
        .then((req) => {
            ...
            newSchema.keyspace = req.keyspace;
            ...
            const cqlTables = newSchemaInfo.cqlTables;
            ...
            const doCreateTables = () => {
                ...
                .then(() => P.each(Object.keys(cqlTables),
                    (cqlTableName) => this._createTable(req, cqlTables[cqlTableName])))
    ...
    _createTable(req, schema) {
        if (!schema.attributes) {
            throw new Error(`No attribute definitions for table ${schema.columnfamily}`);
        }
        ...
        let cql = `create table if not exists ${schema.cqlTablePath} (`;
        ...
        this.log('warn/table/cassandra/createTable', {
            message: `Creating CF ${schema.cqlTablePath}`,
            columnfamily: schema.columnfamily,
