...
dbu.makeSchemaInfo = function makeSchemaInfo(schema, isMetaCF) {
    ...
    const knownKeys = {
        ...
        revisionRetentionPolicy: true,
        table: true,
        options: true,
        _version: true,
        _config_version: true,
        _backend_version: true,
        version: true,
    };

    Object.keys(psi).forEach((key) => {
        if (!knownKeys[key]) { console.log('extra schema key', key, psi); }
    });
    ...
    // XXX: Only define these at the top level & convert once?
    psi.conversions = {};
    ...
    psi.columnfamily = 'hash_0';

    psi.cqlTables = {
        hash_0: new InternalSchema(psi)
    };
    const rangeKeys = psi.index.filter((key) => key.type === 'range');
    rangeKeys.reverse();
    const rangeSchema = Object.assign({}, schema);
    rangeSchema.index = [];
    schema.index.forEach((idx) => {
        if (idx.type === 'static') {
            return;
        }
        if (idx.type === 'range') {
            // Convert to hash
            idx = Object.assign({}, idx, { type: 'hash' });
        }
        rangeSchema.index.push(idx);
    });
    let level = 1;
    rangeKeys.forEach((key) => {
        // Add cqlTable schemas
        rangeSchema.index = rangeSchema.index.filter((item) =>
            item.attribute !== key.attribute);
        rangeSchema.conversions = psi.conversions;
        rangeSchema.columnfamily = `hash_${level}`;
        level++;
        psi.cqlTables[rangeSchema.columnfamily] = new InternalSchema(rangeSchema);
    });

    return psi;
};

function expected(name) { throw new Error(`Missing ${name}`); }

/**
 * Add internal attributes to a schema, especially those focused on index
 * information.
 *
 * Minimal information needed for a table schema:
 * - attributes
 * - index
 *
 * Also needed in buildGetQuery / buildPutQuery
 * - conversions (shared across tables)
 * - columnfamily (currently passed in req).
 *
 * Other top level attributes
 * - revisionRetentionPolicy
 *     - translate to more actionable per-table attributes, like ttl.
 * - options
 *     - used primarily during table creation
 */
class InternalSchema {
    constructor(opts) {
        this.attributes = opts.attributes || expected("attributes");
        this.index = opts.index || expected("index");
        this.keyspace = opts.keyspace || expected("keyspace");
        this.columnfamily = opts.columnfamily || expected("columnfamily");
        this.conversions = opts.conversions || {};
        this.cqlTablePath = `${dbu.cassID(opts.keyspace)}.${dbu.cassID(opts.columnfamily)}`;
        this._addSchemaInfo();
    }
    ...
    _addSchemaInfo() {
        ...
        if (!this.attributes._domain || this.index[0].attribute !== '_domain') {
            this.attributes._domain = 'string';
            this.index.unshift({ attribute: '_domain', type: 'hash' });
        }

        // Create summary data on the primary data index
        this.iKeys = dbu.indexKeys(this.index);
        this.iKeyMap = {};
        this.staticKeyMap = {};
        this.index.forEach((elem) => {
            if (elem.type === 'static') {
                this.staticKeyMap[elem.attribute] = elem;
            } else {
                this.iKeyMap[elem.attribute] = elem;
            }
        });
    }
}
 ...
 * @return {object} queryInfo object with cql and params attributes
 */
dbu.buildPutQuery = (req, schema, noConvert) => {
    if (!schema) {
        throw new Error(`Expected schema for table ${req.table}`);
    }
    ...
    if (!haveNonIndexNonNullValue || query.if === 'not exists') {
        ...
        cql = `insert into ${schema.cqlTablePath} (${proj}) values (`;
        ...
        cql += `update ${schema.cqlTablePath}`
 ...
 * @return {object} queryInfo object with cql and params attributes
 */
dbu.buildGetQuery = (req, schema, options) => {
    options = options || {};
    const query = req.query;
    if (!query) {
    ...
    let cql = `select ${projCQL} from ${schema.cqlTablePath}`;
