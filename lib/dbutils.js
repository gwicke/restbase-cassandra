...
dbu.makeSchemaInfo = function makeSchemaInfo(schema, isMetaCF) {
    ...
    psi._columnfamily = 'hash_0';

    this._addSchemaInfo(psi);

    psi.hashLevels = [psi];
    const rangeKeys = psi.index.filter((key) => key.type === 'range');
    rangeKeys.reverse();
    let rangeSchema = Object.assign({}, schema);
    rangeSchema.index = [];
    ...
    rangeKeys.forEach((key) => {
        rangeSchema = Object.assign({}, rangeSchema);
        // Add hashLevel schemas
        rangeSchema.index = rangeSchema.index.filter((item) =>
            item.attribute !== key.attribute);
        rangeSchema.conversions = psi.conversions;
        rangeSchema._columnfamily = `hash_${level}`;
        level++;
        psi.hashLevels.push(dbu._addSchemaInfo(rangeSchema));
    });

    return psi;
};

dbu._addSchemaInfo = function _addSchemaInfo(psi) {
    // Prefix a _domain attribute to each hash key, so that we can share CFs
    // between groups of domains
    if (!psi.attributes._domain || psi.index[0].attribute !== '_domain') {
